...
- **Roadmap / research** — `memo.txt` (repo root) is the author's master
  plan. It classifies every candidate (Phase 1/2/3, "make / don't make"),
  gives Windows implementation candidates, and contains a worked spec for
  `say`. Read the section for `<command>` and pull only what is relevant.
  Treat the memo as a starting hypothesis, not a spec — verify against the
  real macOS/BSD command's behavior. **`memo.txt` is gitignored (a local
  planning doc, not committed)** — if it's absent in your checkout, ask the
  user for the spec or proceed from the macOS/BSD man page.
...
- Open `memo.txt`; read the row/spec for `<command>`.
